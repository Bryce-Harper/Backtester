...
with a green **API ok** badge, confirming the frontend can reach
the backend, and a price-data panel for loading historical bars.

## Market data pipeline

Price data comes from Yahoo Finance via [yfinance](https://github.com/ranaroussi/yfinance)
(free, no API key, requires internet access on the machine running the
backend).

```
GET /api/prices/{symbol}?start=YYYY-MM-DD&end=YYYY-MM-DD&interval=1d
```

- `start` / `end` default to the trailing year; `interval` is one of
  `1d`, `1wk`, `1mo`.
- Bars are normalized to `date, open, high, low, close, volume`
  (adjusted prices) in `backend/app/services/market_data.py`.
- Responses are cached as CSV under `backend/data/cache/` keyed by
  symbol, interval, and date range, so repeated queries don't re-hit
  the provider. Delete that directory to force fresh downloads.
- The provider is a plain function passed into `get_price_history`, so
  a different data source (or a test fake) can be swapped in without
  touching the router.

Example:

```bash
curl "http://localhost:8000/api/prices/AAPL?start=2025-07-01&end=2026-07-01&interval=1d"
```
